...
const ExerciseCard: React.FC<ExerciseCardProps> = ({
  ...
  const { progress, isCompleted, isAttempted, attemptCount } =
    useExerciseProgressById(exercise.id);
  ...
  const getStatusBadge = () => {
    ...
    const shouldBeCompleted =
      isCompleted || progress?.lastAttempt?.all_correct === true;

    // Ưu tiên kiểm tra isCompleted trước (nếu đã hoàn thành thì không cần kiểm tra partial)
    if (shouldBeCompleted) {
      console.log(
        `Exercise ${exercise.id}: Returning Hoàn thành (shouldBeCompleted: ${shouldBeCompleted})`
      );
      return (
    ...
    if (isAttempted && progress?.bestScore) {
      ...
      console.log(
        `Exercise ${exercise.id}: hasPartialSuccess =`,
        hasPartialSuccess
      );
  ...
  const getDifficultyBadge = () => {
    const difficultyLabel = exerciseService.getDifficultyLabel(
      exercise.difficulty
    );
    const difficultyColor = exerciseService.getDifficultyColor(
      exercise.difficulty
    );
              ...
              <HStack spacing={4} fontSize="xs" color={textSecondary}>
                ...
                {progress?.bestScore && (
                  <HStack spacing={1}>
                    <Icon as={FiTarget} />
                    <Text>
                      {getCorrectAnswersCount(progress.bestScore.results)}/
                      {getResultsCount(progress.bestScore.results)} test cases
                    </Text>
                  </HStack>
                )}
              </HStack>
          ...
          <Box>
            ...
            >
              <Text fontSize="xs" fontFamily="mono" color="text.primary">
                {typeof exercise.test_cases[0] === "object" &&
                exercise.test_cases[0] !== null
                  ? `Input: ${
                      (exercise.test_cases[0] as TestCase).input || "N/A"
                    }`
